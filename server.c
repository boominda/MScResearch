...
PT_THREAD(handle_connection(struct psock *p))
{
   ...
   *          * string.
   *             */
  PSOCK_SEND_STR(p, "Hi This is my certificate\n");
   ...
   *             * buffer we're using.
   *                */
  printf("Received following from client: %s",buffer);
  printf("Client is Authenticated \n");
  PSOCK_SEND_STR(p, "Hi client, this is your shared secret s\n");
  memset(buffer,0,sizeof(buffer));
  PSOCK_READTO(p, '\n');
  printf("Received following from client: %s",buffer);
  printf("Client Accepted my shared key \n");
  printf("Connected sent to client \n");
